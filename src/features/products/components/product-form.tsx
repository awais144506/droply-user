...
import { Package, Calculator, Settings2, Plus, Trash2, PlusCircle, AlertCircle, Loader2 } from "lucide-react";
...
export function ProductForm({ 
    ...
    // Fetch live products for the recipe dropdown
    const { data: products = [], isLoading: isProductsLoading } = useProducts(branchId);

    // Map database items to React-Select format
                ...
                <Card>
                    ...
                    <CardContent className="pt-4 space-y-4">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            ...
                            <div className="space-y-1.5">
                                <label className="text-xs font-bold text-slate-700 uppercase tracking-wide">SKU / Code *</label>
                                <input
                                    type="text"
                                    placeholder="e.g. BOT-19L"
                                    {...register("sku")}
                                    className={`w-full h-10 px-3 rounded-xl border bg-slate-50 text-sm focus:outline-none focus:ring-2 focus:bg-white ${errors.sku ? "border-rose-300 focus:ring-rose-500" : "border-slate-200 focus:ring-sky-500/20 focus:border-sky-500"}`}
                                />
                                    ...
                                    render={({ field }) => (
                                        <ReactSelect
                                            ...
                                            classNames={{
                                                ...
                                                control: (state) => `flex min-h-10 w-full items-center justify-between rounded-xl bg-slate-50 border px-3 ${errors.category ? 'border-rose-300' : state.isFocused ? 'border-sky-500 ring-2 ring-sky-500/20' : 'border-slate-200'} hover:bg-white transition-colors text-sm cursor-pointer`
                                            }}
                                        />
                                    )}
                                />
                                ...
                                <Controller
                                    ...
                                    render={({ field }) => (
                                        <ReactSelect
                                            ...
                                            classNames={{
                                                ...reactSelectClassNames,
                                                control: (state) => `flex min-h-10 w-full items-center justify-between rounded-xl bg-slate-50 border px-3 ${errors.trackingType ? 'border-rose-300' : state.isFocused ? 'border-sky-500 ring-2 ring-sky-500/20' : 'border-slate-200'} hover:bg-white transition-colors text-sm cursor-pointer`
                                            }}
                                        />
                                    )}
                                />
                                            ...
                                            <Controller
                                                ...
                                                render={({ field: selectField }) => (
                                                    <ReactSelect
                                                        ...
                                                        classNames={{
                                                            ...reactSelectClassNames,
                                                            control: (state) => `flex min-h-10 w-full items-center justify-between rounded-xl bg-slate-50 border px-3 ${errors.recipeItems?.[index]?.rawMaterialId ? 'border-rose-300' : state.isFocused ? 'border-sky-500 ring-2 ring-sky-500/20' : 'border-slate-200'} hover:bg-white transition-colors text-sm cursor-text`
                                                        }}
                                                    />
                                                )}
                                            />
