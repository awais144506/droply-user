...
import { StockItem } from "../api/use-dashboard";
...
export function QuickSale({ items }: { items: StockItem[] }) {
  ...
  // Decoupled state specifically for the print template
  ...
  const handlePrintNew = () => {
    ...
    // Timeout ensures React flushes the state to the DOM before the print dialog locks the thread
    setTimeout(() => {
      window.print();
      setQuantity(1);
    }, 100);
          ...
          {activeTab === "NEW" ? (
            <div className="flex flex-col h-full">
              <div className="space-y-4 flex-1">
                <div>
                  ...
                  <select 
                    ...
                    className="w-full h-10 px-4 mt-1.5 rounded-xl border border-slate-200 bg-slate-50 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500/20"
                  >
                    {items.map(item => (
              ...
              <div className="mt-6 pt-4 border-t border-slate-100 flex items-center justify-between">
                ...
                <Button onClick={handlePrintNew} className="bg-slate-900 hover:bg-slate-800 text-white h-12 px-6 rounded-xl shadow-md cursor-pointer">
                      ...
                      {sale.status === "COMPLETED" ? (
                        <>
                          <Button 
                            ...
                            className="flex items-center gap-1 text-[10px] font-bold text-sky-600 hover:text-sky-700 bg-sky-50 hover:bg-sky-100 px-2 py-1 rounded transition-colors cursor-pointer"
                          >
                            <Printer className="h-3 w-3" /> Reprint
                          </Button>
                          <Button 
                            onClick={() => handleVoidSlip(sale.id)}
                            className="flex items-center gap-1 text-[10px] font-bold text-rose-600 hover:text-rose-700 bg-rose-50 hover:bg-rose-100 px-2 py-1 rounded transition-colors cursor-pointer"
                          >
