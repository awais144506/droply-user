...
// T represents the data type (e.g., ProductItem, UserItem)
...
export default function DataTable<T>({ data, columns, emptyMessage = "No items found." }: DataTableProps<T>) {
    return (
                            ...
                            <tr key={rowIndex} className="hover:bg-slate-50/50 transition-colors group">
                                {columns.map((col, colIndex) => (
                                    <td key={colIndex} className={`px-4 py-4 ${col.className || ""}`}>
                                        {/* If a custom render function is provided, use it. Otherwise, print the raw data. */}
                                        {col.render 
                                            ? col.render(item) 
